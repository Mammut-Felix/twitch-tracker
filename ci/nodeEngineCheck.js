...
class NodeEngineCheck {
  ...
  async getNodeVersion({ files, ignoredFilePaths = [], clean = false }) {
    ...
    for (const file of files) {
      const shouldFileBeIgnored = ignoredFilePaths.some(path => path.test(file))
      ...
      try {
        ...
        console.error(`Failed to parse package at ${file} with error ${error}`)
        continue
      }
    }
  ...
  try {
    ...
    if (higherVersions.length > 0) {
      ...
      for (const higherVersion of higherVersions) {
        // Get package name
        const _package = higherVersion.file
          .split('node_modules/')
          .pop()
